...
sealed class SseEvent {
    ...
    data class SessionStatus(
        val sessionId: String,
        val status: dev.leonardo.ocremoteplus.domain.model.SessionStatus
    ) : SseEvent()
...
data class Project(
    ...
    val displayName: String
        get() = name?.takeIf { it.isNotEmpty() }
            ?: worktree.takeIf { it.isNotEmpty() }?.let { dev.leonardo.ocremoteplus.util.PathUtils.fileName(it.trimEnd('/', '\\')) }?.takeIf { it.isNotEmpty() }
            ?: path.takeIf { it.isNotEmpty() }?.let { dev.leonardo.ocremoteplus.util.PathUtils.fileName(it.trimEnd('/', '\\')) }?.takeIf { it.isNotEmpty() }
            ?: id.take(8)
}
